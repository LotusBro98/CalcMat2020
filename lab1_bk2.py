...
n = len(y0)
En = np.eye(n, n)
def impl_runge_kutt(x, y, h, a, p=0.5, lr=1e-1, eps=1e-1, step_clamp=1e3):
    k = np.zeros((n, s), dtype=np.float64)
    for i in range(s):
        ...
        J_ = En - h * a_d * J(x, y, a)
        if np.abs(np.linalg.det(J_)) > 0.1 * np.linalg.norm(J_):
            J_1 = np.linalg.inv(J_)
            k[:,i] = np.matmul(J_1, F(x, y + h * np.sum(A_lower[i] * k, axis=-1), a))
        else:
            k[:, i] = F(x, y + h * np.sum(A_lower[i] * k, axis=-1), a)
        # Невязка
        f = k[:,i] - F(x, y + h * np.sum(A[i] * k, axis=-1), a) # -> 0
        while np.linalg.norm(f) > eps:
            # Шаг метода Ньютона
            # J_ = (df / dki)
            J_ = En - h * a_d * J(x, y + h * np.sum(A_lower[i] * k, axis=-1), a)
            if np.abs(np.linalg.det(J_)) > 0.01 * np.linalg.norm(J_):
                J_1 = np.linalg.inv(J_)
                step = -p * np.matmul(J_1, f)
            else:
                # Если det(J_) слишком мал, метод Ньютона будет неустойчив,
                # продолжим приближение методом градиентного спуска,
                # пока не станет доступен метод Ньютона
                step = -lr * np.matmul(J_, f / np.linalg.norm(f))
            ...
            k[:,i] += step
            # Вычисляем невязку
            f = k[:,i] - F(x, y + h * np.sum(A[i] * k, axis=-1), a)
